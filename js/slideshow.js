...
function highlight()
{
	...
	this.slider = function(cont,nav,t)
	{
		...
		for ( var i = 0; i < total.length; i++ )
		{
			...
			//
			if ( this.images[i].complete && document.all )
			{
				this.images[this.z].style.zIndex = this.z + 1;
				this.counterpreloader+= 1;
				if ( this.counterpreloader >= total.length )
				{
					this.images[0].style.zIndex = total.length + 1;
					this.fadeElement(this.images[0],true);
					this.counterpreloader = 0;
					this.swithImage(this.currentimage);
					// this.container.style.background = "white";
					this.loadButtons(nav);
				}
			}
			//
